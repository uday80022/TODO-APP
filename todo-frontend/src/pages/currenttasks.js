...
const CurrentTasks = ({ alltodos, setTodos }) => {
  ...
  const addTodo = async () => {
    if (!task) return;
    try {
      const response = await axios.post("https://todo-app-backend-jig8.onrender.com/addtodo", {
        user_id: user.id,
        task,
        priority,
  ...
  const changePriority = async (id, priority) => {
    try {
      await axios.put(`https://todo-app-backend-jig8.onrender.com/updatetask/${id}`, { priority });
      setTodos(
  ...
  const editTask = async (id) => {
    try {
      const response = await axios.put(
        `https://todo-app-backend-jig8.onrender.com/updatetask/${id}`,
        {
          editedTask,
        }
      );
  ...
  const updateOrder = async (id, direction) => {
    try {
      ...
      const response = await axios.put(`https://todo-app-backend-jig8.onrender.com/updateorder`, {
        id,
        user_id: user.id,
        swap_sort_id,
        current_sort_id,
  ...
  const deleteTodo = async (id) => {
    try {
      await axios.put(`https://todo-app-backend-jig8.onrender.com/delete/${id}`);
      setTodos(
        alltodos.map((todo) => {
